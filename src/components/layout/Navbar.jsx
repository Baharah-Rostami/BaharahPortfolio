...
function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 z-1000 w-full py-4 transition-all duration-300 ${isScrolled
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
              ...
              aria-label="home">{personalInfo.name.split(' ')[0]}</button>
          </div>
          ...
          <div className="hidden md:flex items-center gap-2">
            <button className="px-7 py-3.5 bg-white text-[#212121] font-medium text-base rounded-[17px] border border-white hover:bg-white/90 transition-all duration-300 "
              onClick={() => handleNavClick('contact')}>Hire Me</button>
          </div>
          {/* mobile menu butto  */}
          ...
          {Nav_Links.map((link) => (
            <button key={link.id}
              ...
              className={`block w-full text-left px-4 py-3 rounded-lg font-medium transition-all duration-300 ${activeSection === link.id
                ...
                : "text-white/70 hover:text-white hover:bg-white/40"}`}>
              {link.label}
            </button>
          ))}
          <button
