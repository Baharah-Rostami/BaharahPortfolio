...
export default function ProjectCard({project}) {
  const {title, description, image, category, technologies,demoUrl, githubUrl} = project;
  return (
    <div className="group relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-primary/30 transition-all duration-300">
      <div className="relative h-64 overflow-hidden">
        ...
        <div className="absolute bottom-4 right-4 flex items-center gap-3">
          ...
          {demoUrl && (
            <a href={demoUrl} target="_blank" rel="no repper" title="view demo"
           ...
           {githubUrl && (
            <a href={githubUrl} target="_blank" rel="no repper" title="view code"
            className="p-2.5 bg-white/10 backdrop-blur-md rounded-lg border border-white/20 hover:bg-primary/30 hover:border-primary/50 transition-all duration-300 hover:scale-110">
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {technologies.map((tech, index)=>(
            <span key={index} className="px-3 py-1 text-sx font-medium text-primary bg-primary/10 border border-primary/20 rounded-lg hover:bg-primary/20 transition-colors duration-200 ">{tech}</span>
          ))}
        </div>
